...
cpp = f"""
    ...
    int main(int argc, char* argv[]) {{
      ...
      auto sub = Kokkos::subview(v, Kokkos::ALL(), Kokkos::ALL(), 3);
...
def testLayout(generateCMakeProject, runGDB):
    ...
    r, resultStr = runGDB(
        ...
        py sub = gdb.parse_and_eval('sub')
        py print(getKokkosViewLayout(sub))
        quit

        """,
        fGDB, f"{fTest.realpath().strpath}"
        )
    assert resultStr == layoutSub, f"Wrong output layout: {r.stdout}"
...
def testExtent(generateCMakeProject, runGDB):
    ...
    r, resultStr = runGDB(
        ...
        py sub = gdb.parse_and_eval('sub')
        py print(getKokkosViewExtent(sub))
        quit
    ...
    assert (result == np.array(shapeSub)).all(), f"Wrong output extent: {r.stdout}"
...
def testStrides(generateCMakeProject, runGDB):
    ...
    r, resultStr = runGDB(
        ...
        py sub = gdb.parse_and_eval('sub')
        py print(getKokkosViewStrides(sub))
        quit
    ...
    expected = np.cumprod(expected)[::-1][:-1]
...
def testTraits(generateCMakeProject, runGDB):
    ...
    r, resultStr = runGDB(
        ...
        py sub = gdb.parse_and_eval('sub')
        py print(getKokkosViewTraits(sub))
        quit

        """,
        fGDB, f"{fTest.realpath().strpath}"
        )
    assert re.match(r"^Kokkos::ViewTraits<float\*\*, \s*Kokkos::LayoutStride.*>$",
                    resultStr) is not None,\
...
def testSpan(generateCMakeProject, runGDB):
    ...
    r, resultStr = runGDB(
        ...
        py sub = gdb.parse_and_eval('sub')
        py print(getKokkosViewSpan(sub))
        quit

        """,
        fGDB, f"{fTest.realpath().strpath}"
        )
    # strided subview will have same span as the whole full
...
def testPrintView(generateCMakeProject, runGDB):
    ...
    result = np.array(resultStr.split()).astype(float).reshape(shapeSub)
    expected = np.arange(span).astype(float).reshape(shape)[:, :, 3]
